...
void lcd_i2c_init( uint8_t address )
{
   ...
   Delay_ms( 80 ); // Esta accion requiere de al menos 40 ms

   lcd_i2c_write_instruction( LCD_FunctionSet4bit );
   Delay_ms( 80 ); // Esta accion requiere de al menos 80 ms
...
void lcd_i2c_write( uint8_t byte )
{
   ...
   I2C1_Wr( payload );
   I2C1_Stop();
   // I2C1_WriteSlave( __ADD, &payload, 1 );
   Delay_us( 1 );
...
/*******************************************************************************
 ...
 ---------------------------
 Coloca el cursor en la posicin columna y fila. Empezando con 1.
...
void lcd_i2c_setCursor( uint8_t col, uint8_t row )
{
   static const uint8_t lineStartPosition[] = {
       0x00, 0x40, 0x14, 0x54 };
   // Evita leer una direccin fuera del arreglo
   if ( row > 3 )
      return;
